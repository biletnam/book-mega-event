...
$(document).ready(function () {
    ...
    $('#Kandivali').click(function () {
        $('#KandivaliGamesTab').show();
        $('#LowerParelTab').hide();
        $('#AndheriTab').hide();
        $('#KandivaliCalendar').show();
        $('#LowerParelCalendar').hide();
        $('#AndheriCalendar').hide();
        $('#EventPlaceAddress').text('Goregaon Sports Club, C3, Link Road, Near Toyota showroom, Malad West, Mumbai - 400064.');
    });
    ...
    $(document.body).on('click', '.available', function () {
        ...
        $.ajax({
           ...
           "<span id='DivScrId' style='font-size: 9px; display: none;'></span><asp:HiddenField ID='hdntimingid' runat='server' /></td></tr></table></td></tr></table></div>"));
                ...
                //$("<li class='txtcolumn'></li>").val(value.Id).html("<div class='txtcolumn'><b>Address:</b> 101, Roha Orion, 16th Road, Near Mini Punjab Restaurant, Kandivali West, Mumbai</div>").appendTo(mytable);
    ...
    function LoadDataGetDataforSpykarChamber() {
        ...
        $.ajax({
            type: "POST",
            url: "CluehuntwebService.asmx/GetDataForNoEscapeGame",
            data: "{pageId:'" + pageId + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (data) {
                ...
                $("#mytable").find('tr').each(function() {
                    $(this).find('td').eq(1).find('.available').addClass("disabled");
                    $(this).find('td').eq(1).find('.disabled').removeClass("available");
                    $(this).find('td').eq(1).find('.disabled').empty();
                    $(this).find('td').eq(1).find('.disabled').append("<span>Booking Closed</span>");
                });
            },
            error: function (result) {
